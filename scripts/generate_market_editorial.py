...
def build_briefs(
    ...
    equity_items = [
        ...
        evidence("btc-30d-relative", "比特幣 30 日報酬", btc_30d, signed_pct(btc_30d), f"相對標普差 {signed_pct((btc_30d - sp_change) if btc_30d is not None and sp_change is not None else None)}", snapshot.get("date"), direction(btc_30d, 0.08), btc_return_source_count, [source("雙來源 BTC 日 K", "data/daily/timescale_intelligence.json")], "用來判斷 BTC 是高 beta 風險資產，或正在走獨立路徑。", "跨市場"),
    ]

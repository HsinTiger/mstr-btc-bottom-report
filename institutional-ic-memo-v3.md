...
> 範圍：BTC 現貨週期倉、BTC 合約 W1/M1 波段、MSTR / DAT 風險閘門  
...
| 問題 | 機構級裁決 |
|---|---|
| BTC 是否可做多？ | 可，但只能依狀態機。現貨週期倉等待 S2 投降或 S3′/S4 確認；合約只做 W1/M1 小型衛星。 |
| 今天是否可開新合約多？ | **否。** S1 仍是主判定，投降/確認條件未齊；Strategy 賣幣事件使機構閘需先紅隊覆核。 |
| 週線策略 | W1 只吃 S2/S3 後 2–8 週反彈，單筆風險 0.5%，槓桿 ≤2x。 |
| 月線策略 | M1 只吃 S3′/S4 右側 3–12 個月趨勢，等效曝險 ≤1.3x，永續不得作核心工具。 |
...
| 狀態 | 條件摘要 | 現貨 | W1 合約 | M1 等效曝險 | MSTR |
|---|---|---:|---:|---:|---|
| S1 深熊下跌 | 200DMA 下、投降未齊 | 等觸發 | 0 | 0 | 紅燈覆核，不加 |
| S2 投降 | 價值觸發＋實現虧損＋極端情緒/費率 | 第 1 等份 | 可 0.5% 風險 | 0 | 只允許 IC 批准 |
| S3 投降後築底 | ≥14 天不創新低＋結構確認 | 可補 | 可 W1 | 小觀察 | 需 mNAV 與賣幣事件降級解除 |
| S3′ 無投降築底 | ≥45 天不創新低＋ETF/週線確認 | 1/3 DCA | 不追短線 | 可啟動 1/3 | 仍需紅燈解除 |
| S4 確認上行 | ETF/200DMA/mNAV/溫度齊 | 第 2 等份 | 只做回踩 | 可加至 1.3x | 解除紅燈後才可恢復 |
| S5 狂熱 | 溫度過熱、情緒/槓桿過高 | 出場 | 禁開 | 降曝險 | 降曝險 |
...
| 風險 | 等級 | 觸發 | 影響 | 控制 |
|---|---|---|---|---|
| 清算風險 | Critical | 槓桿過高、清算價貼近止損 | 看對方向仍出局 | W1≤2x、M1≤1.3x、清算距離硬檢查 |
| MSTR 賣幣 / monetization | Critical | Strategy 實際賣 BTC 或啟動 monetization | 財庫敘事折價、mNAV 壓縮 | 機構閘紅燈；凍結 MSTR 加倉與合約原型 B/M1 依賴 |
| ETF 順週期流出 | High | 連 5 日淨流出、IBIT 同步流出 | 假突破、流動性抽離 | 禁新合約；M1 降 1/2 |
...
| 桶 | 用途 | 上限 | 工具 |
|---|---|---:|---|
| Core | 週期倉 | 主要資金 | BTC 現貨、MSTR/BMNR 現股（受閘門） |
| Tactical | W1/M1 衛星 | 合約虧損預算 5% 硬頂 | BTC 永續、CME micro、LEAP |
...
- W1：+1R 出 1/3，+2R 或 50DMA 再出 1/3，剩餘追蹤。
- M1：+3R / +5R 各出 1/3，剩餘跟 10 週均線。
...
1. **不開新合約多單**：S1/S2/S3 條件未齊，且 MSTR 機構閘需重評。
...
5. **下次可行開倉只剩兩路**：S2 投降後 W1，或 S3′/S4 確認後 M1；中間灰區只觀察。

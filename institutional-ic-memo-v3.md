...
> **封存聲明（2026-07-27）**：此文件保留歷史 IC 研究脈絡，不再代表 active product。現行 repo 聚焦四週期資料分析、趨勢洞察與可反證觀點；入口見 `README.md` 與 `index.html`。
...
| 指標 | 用途 | 紅燈 / 降權 |
|---|---|---|
| 普通股市值／普通股淨值 | 普通股估值安全邊際 | ≤1.0 倍只是折價背景，仍非單獨買點 |
| 企業價值／BTC 總值 | 企業層融資飛輪 | 與普通股便宜度分開判斷 |
| 特別股融資扭曲旗標 | 防特別股發行機械式墊高 mNAV | 觸發時所有 mNAV 樂觀訊號降權 |
| 明示固定義務覆蓋月數 | 美元儲備能支撐多久 | <12 月直接紅燈 |
| 7 日賣幣壓力倍數 | 是否因固定義務承壓 | 未知或 >2 倍均凍結合約加碼 |
| 每股比特幣含量 | 檢查是否真正增厚 | BTC 減且稀釋增＝惡化 |
| STRC 優先股折價信任票 | 資本市場對優先層的信任 | >5% 時全體 mNAV 訊號降權 |
...
結論：MSTR 持有分析不能再把「ASU 2023-08 讓 BTC 按市價入帳」簡化成「GAAP NAV 等於 BTC NAV」。小倉 2.5x 合約要求更高：「普通股市值／普通股淨值」必須扣淨遞延稅負債；STRC 折價、賣幣紅燈或 SEC/10-Q 未覆核時，mNAV 回升不視為安全邊際。BMNR 也不能只套「ETH 版 MSTR」：必須獨立檢查每股 ETH 增厚效率、稀釋速度與任何優先順位融資。
